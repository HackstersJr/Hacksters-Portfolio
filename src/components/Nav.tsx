    ...
    <nav className="fixed top-0 left-0 right-0 z-50 py-6 transition-all duration-300" id="navbar">
      <div className="container mx-auto px-4">
        <div className="glass-nav rounded-full px-4 md:px-8 py-3 md:py-4 max-w-2xl mx-auto">
          {/* Desktop Nav */}
          <ul className="hidden md:flex items-center justify-center space-x-8 font-chillax">
            {navItems.map((item) => (
              <li key={item.id}>
                <a 
                  href={`#${item.id}`} 
                  onClick={(e) => { e.preventDefault(); scrollToSection(item.id); }} 
                  className="nav-link px-4 py-2 text-sm font-medium transition-all duration-300 hover:text-cyan-400 text-gray-300 active:scale-95"
                >
                  {item.label}
                </a>
              </li>
