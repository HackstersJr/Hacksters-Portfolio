...
export default function DomeGallery({
  ...
  // Auto-rotation when idle: slow, one-direction spin. Pauses while dragging or when image is focused.
  useEffect(() => {
    let raf: number | null = null;
    const SPEED_DEG_PER_SEC = 0.5 / 60; // degrees per second (very slow). Small value to make one-direction spin
    const lastTimeRef = { t: performance.now() };

    const step = () => {
      const now = performance.now();
      const dt = (now - lastTimeRef.t) / 1000;
      lastTimeRef.t = now;

      // if user is interacting or an image is open, don't auto-rotate
      if (!draggingRef.current && !focusedElRef.current && !openingRef.current) {
        // increment Y rotation only (one-direction)
        rotationRef.current.y = wrapAngleSigned(rotationRef.current.y + SPEED_DEG_PER_SEC * dt * 360);
        applyTransform(rotationRef.current.x, rotationRef.current.y);
      }
  ...
  useGesture(
    {
      onDragStart: (state: { event: Event }) => {
        ...
        pointerTypeRef.current = ((evt as PointerEvent).pointerType as 'mouse' | 'pen' | 'touch') || 'mouse';
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Removed radial fade overlays so the dome shows fully behind the text */}

          {/* Top/bottom gradient strips removed so the nav area is not darkened */}
